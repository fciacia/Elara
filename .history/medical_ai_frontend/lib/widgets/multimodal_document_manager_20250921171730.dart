    ...
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        if (kIsWeb) {
          ...
          final success = await documentProvider.uploadDocumentsWeb(files);
          if (mounted) {
          ...
          final success = await documentProvider.uploadDocuments(files);
          if (mounted) {
            messenger.showSnackBar(
              SnackBar(
                content: Text(
  ...
  String _getDocumentTypeDisplayName(DocumentType type) {
    switch (type) {
      case DocumentType.labReport:
        return 'lab_reports'.tr;
      case DocumentType.prescription:
        return 'prescriptions'.tr;
      case DocumentType.discharge:
        return 'discharge_papers'.tr;
      case DocumentType.xray:
        return 'xray_imaging'.tr;
      case DocumentType.consultation:
        return 'consultation_notes'.tr;
      case DocumentType.insurance:
        return 'insurance_forms'.tr;
      case DocumentType.general:
        return 'general_documents'.tr;
    }
  }
